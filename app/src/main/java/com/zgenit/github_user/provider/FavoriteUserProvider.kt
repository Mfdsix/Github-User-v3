package com.zgenit.github_user.provider
...
import com.zgenit.github_user.db.DatabaseContract.AUTHORITY
...
class FavoriteUserProvider : ContentProvider() {
    ...
    fun isExists(uri: Uri, userId: Int): Boolean {
        var exist = false
        when(FAVORITE_USER){
            sUriMatcher.match(uri) -> exist = favoriteUserHelper.isExists(userId)
        }
        return exist
    }
}
